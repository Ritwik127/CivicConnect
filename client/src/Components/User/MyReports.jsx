...
import { fetchMyReports, getReadableErrorMessage } from '../../lib/api';
...
function formatStatus(status) {
  return String(status || 'open').replaceAll('_', '-').toUpperCase();
}
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black tracking-widest border opacity-90 ${styles[normalizedStatus] || styles.OPEN}`}>
      {normalizedStatus}
    </span>
  );
};
...
const MyReports = () => {
  ...
  useEffect(() => {
    ...
    async function loadReports() {
      try {
        ...
        if (isMounted) {
          setErrorMessage(getReadableErrorMessage(error, 'Unable to load your reports.'));
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
  ...
  const filteredReports = useMemo(
    () => reports.filter((report) => filter === 'ALL' || formatStatus(report.status) === filter),
    [filter, reports],
  );
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div className={`flex items-center gap-3 pl-4 sm:pl-6 border-l ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
              <div className="text-right hidden sm:block">
                <p className="text-sm font-bold">{user?.full_name || 'Citizen'}</p>
              ...
              <div className="w-11 h-11 !bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl flex items-center justify-center font-bold text-white shadow-lg shadow-blue-900/20">
                {(user?.full_name || 'CI').split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase()}
              </div>
            </div>
          </div>
        </header>
          ...
          <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div>
                ...
                <p className={`mt-1 font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Track the live status of your submitted civic issues.</p>
              </div>
            ...
            <div className={`flex flex-wrap items-center gap-2 p-1.5 rounded-2xl w-max border shadow-sm ${isDark ? '!bg-slate-900/50 border-slate-800' : '!bg-white border-slate-200/60'}`}>
              {['ALL', 'OPEN', 'IN-PROGRESS', 'ACKNOWLEDGED', 'RESOLVED'].map((value) => (
                <button key={value} onClick={() => setFilter(value)} className={`px-5 py-2.5 rounded-xl text-xs font-black tracking-widest uppercase transition-all ${filter === value ? (isDark ? '!bg-slate-800 text-white shadow-md' : '!bg-blue-100 text-slate-900 shadow-sm') : (isDark ? '!bg-slate-800 text-slate-500 hover:text-slate-300 hover:!bg-slate-800/50' : '!bg-slate-100 text-slate-500 hover:text-slate-700 hover:!bg-slate-50')}`}>
                  {value}
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {isLoading ? (
                <div className={`border p-10 rounded-[2rem] flex items-center justify-center gap-3 ${isDark ? '!bg-slate-900/80 border-slate-800' : '!bg-white/90 border-slate-200/60'}`}>
              ...
              {filteredReports.map((report) => {
                const categoryKey = normalizeCategory(report.category);
                      ...
                      <div className="flex items-start md:items-center gap-5">
                        ...
                        <div>
                          <div className="flex items-center gap-3 mb-1">
                            <h3 className="font-bold text-lg">{report.category || report.title}</h3>
                            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">ID: {report.tracking_code}</span>
                          </div>
                          <div className={`flex flex-wrap items-center gap-4 text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                            <span className="flex items-center gap-1.5"><MapPin className="w-4 h-4" /> {report.address || 'Address unavailable'}</span>
                            <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4" /> {formatDate(report.created_at)}</span>
                          </div>
                        </div>
                      </div>
