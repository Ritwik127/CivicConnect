...
const MOCK_TASKS = [
  { 
    ...
    location: 'Oxford Street, Downtown', priority: 'High', status: 'ASSIGNED', 
    slaDeadline: 'Today, 2:00 PM', distance: '1.2 km away'
  },
  { 
    id: 'TSK-9045', issueId: 'ISS-9045', category: 'Traffic Light Fix', 
    location: 'Main St & 5th Ave', priority: 'Critical', status: 'IN-PROGRESS', 
    slaDeadline: 'Today, 12:30 PM', distance: '0.8 km away'
  },
  { 
    id: 'TSK-9011', issueId: 'ISS-9011', category: 'Fallen Tree Removal', 
    location: 'Central Park North', priority: 'Medium', status: 'RESOLVED', 
    slaDeadline: 'Completed', distance: '3.5 km away'
  },
  { 
    id: 'TSK-8992', issueId: 'ISS-8992', category: 'Sewer Leak', 
    location: 'West End Blvd', priority: 'High', status: 'ASSIGNED', 
    slaDeadline: 'Tomorrow, 10:00 AM', distance: '4.1 km away'
  }
];
...
const MyTasks = () => {
  ...
  const filteredTasks = MOCK_TASKS.filter(t => {
    ...
    if (filter === 'PENDING') return t.status === 'ASSIGNED' || t.status === 'IN-PROGRESS';
          ...
          <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-2">
              <div>
                ...
                <p className={`mt-1.5 text-base font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Manage your field assignments and update statuses.</p>
              </div>
            ...
            <div className="space-y-4">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div className="flex items-start md:items-center gap-5">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300 ${
                        ...
                        {task.status === 'RESOLVED' ? <CheckCircle2 className="w-7 h-7" /> : <AlertTriangle className="w-7 h-7" />}
                      </div>
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          <h3 className="font-bold text-xl">{task.category}</h3>
                          <PriorityBadge priority={task.priority} />
                        </div>
                        <div className={`flex flex-wrap items-center gap-4 text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                          <span className="flex items-center gap-1.5 font-bold"><MapPin className="w-4 h-4" /> {task.location}</span>
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-6 md:w-64 pl-14 md:pl-0">
                      ...
                      <button className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all shadow-md group-hover:!bg-orange-600 group-hover:text-white ${isDark ? '!bg-slate-800/80 text-slate-400' : '!bg-slate-100 text-slate-600'}`}>
                        <ChevronRight style={{ width: '2.4rem', height: '2.4rem' }} strokeWidth={2.5} />
                      </button>
                    </div>
              ...
              {filteredTasks.length === 0 && (
                <div className={`text-center py-24 rounded-[2.5rem] border border-dashed backdrop-blur-xl ${isDark ? '!bg-slate-900/30 border-slate-800' : '!bg-white/50 border-slate-300'}`}>
                  ...
                  <p className={`font-bold text-lg ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>No tasks match this filter.</p>
                </div>
              )}
            </div>
