...
const WorkerSettings = () => {
  ...
  const [formData, setFormData] = useState({
    name: 'Mike Reynolds', email: 'm.reynolds@civic.gov', phone: '+1 234 567 8900',
    emailAlerts: false, smsAlerts: true, pushAlerts: true
  });
    ...
    <div className={`flex h-screen w-screen font-sans overflow-hidden transition-colors duration-500 relative ${isDark ? '!bg-[#0f172a] text-white' : '!bg-[#f8fafc] text-slate-800'}`}>
      ...
      <aside className={`hidden lg:flex flex-col w-64 border-r h-full shrink-0 relative z-20 backdrop-blur-xl transition-all duration-300 ${isDark ? '!bg-[#0f172a]/80 border-slate-800 shadow-lg shadow-black/20' : '!bg-white/80 border-slate-200/80 shadow-sm'}`}>
        ...
        <div className={`p-4 border-t space-y-2 ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
          <button className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold border transition-all text-left ${isDark ? '!bg-orange-500/10 text-orange-400 border-orange-500/20 shadow-[0_0_15px_rgba(249,115,22,0.05)]' : '!bg-orange-50 text-orange-700 border-orange-200 shadow-sm'}`}>
          ...
          <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              ...
              <div className="w-full md:w-64 shrink-0 space-y-2">
                <button onClick={() => setActiveTab('profile')} className={`w-full flex items-center gap-3 p-4 rounded-2xl transition-all font-bold ${activeTab === 'profile' ? (isDark ? '!bg-slate-800 text-white shadow-md' : '!bg-white shadow-sm text-orange-600') : (isDark ? '!bg-slate-800 text-slate-400 hover:!bg-slate-900/50 hover:text-slate-200' : '!bg-orange-50 text-slate-600 hover:!bg-white hover:text-slate-900')}`}>
                  <User className="w-5 h-5" /> Personal Profile
                </button>
                <button onClick={() => setActiveTab('notifications')} className={`w-full flex items-center gap-3 p-4 rounded-2xl transition-all font-bold ${activeTab === 'notifications' ? (isDark ? '!bg-slate-800 text-white shadow-md' : '!bg-white shadow-sm text-orange-600') : (isDark ? '!bg-slate-800 text-slate-400 hover:!bg-slate-900/50 hover:text-slate-200' : '!bg-orange-50 text-slate-600 hover:!bg-white hover:text-slate-900')}`}>
                  <Bell className="w-5 h-5" /> Notifications
                </button>
                <button onClick={() => setActiveTab('security')} className={`w-full flex items-center gap-3 p-4 rounded-2xl transition-all font-bold ${activeTab === 'security' ? (isDark ? '!bg-slate-800 text-white shadow-md' : '!bg-white shadow-sm text-orange-600') : (isDark ? '!bg-slate-800 text-slate-400 hover:!bg-slate-900/50 hover:text-slate-200' : '!bg-orange-50 text-slate-600 hover:!bg-white hover:text-slate-900')}`}>
                  <ShieldCheck className="w-5 h-5" /> Security & Passwords
                </button>
              </div>

              {/* Settings Content Area */}
              <div className={`flex-1 border rounded-[2.5rem] p-8 md:p-12 shadow-xl backdrop-blur-xl transition-all duration-500 ${isDark ? '!bg-slate-900/80 border-slate-800 shadow-black/20' : '!bg-white/90 border-slate-200/60 shadow-slate-200/50'}`}>
                ...
                {activeTab === 'profile' && (
                  <form onSubmit={handleSave} className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                    <div>
                      ...
                      <p className={`font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Update your contact information for dispatch communication.</p>
                    </div>

                    <div className={`flex items-center gap-6 pb-6 border-b ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
                      <div className="w-24 h-24 !bg-gradient-to-br from-orange-500 to-amber-600 rounded-3xl flex items-center justify-center font-bold text-3xl text-white shadow-lg shadow-orange-900/20">
                        MR
                      </div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <div className="relative group">
                          ...
                          <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className={`w-full border rounded-[1.5rem] py-4 pl-12 pr-4 focus:border-orange-500 outline-none transition-all focus:ring-4 focus:ring-orange-500/10 font-medium shadow-inner ${isDark ? '!bg-slate-950/50 border-slate-800 text-slate-200' : '!bg-slate-50/80 border-slate-200 text-slate-900 focus:!bg-white'}`} />
                        </div>
                      </div>
                    ...
                    <div className="pt-4 flex justify-end">
                      <button type="submit" className={`px-8 py-4 rounded-2xl font-bold transition-all flex items-center gap-2 active:scale-95 shadow-xl ${isDark ? '!bg-orange-600 hover:!bg-orange-500 text-white shadow-orange-900/20' : '!bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white shadow-orange-600/30'}`}>
                ...
                {activeTab === 'notifications' && (
                  <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                    <div>
                      ...
                      <p className={`font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Control how dispatch sends you new tasks and alerts.</p>
                    </div>

                    <div className="space-y-6">
                      {[{key: 'emailAlerts', icon: Mail, title: 'Email Alerts', desc: 'Receive task summaries to your inbox.', color: 'blue'},
                        {key: 'smsAlerts', icon: Smartphone, title: 'SMS Alerts', desc: 'Get immediate text messages for Critical priority dispatches.', color: 'emerald'},
                        {key: 'pushAlerts', icon: Bell, title: 'Push Notifications', desc: 'In-app notifications and alerts while working.', color: 'orange'}].map(item => (
                        <div key={item.key} className={`flex items-center justify-between p-6 border rounded-[2rem] group transition-all duration-300 hover:-translate-y-1 shadow-sm ${isDark ? '!bg-slate-950/50 border-slate-800 hover:border-slate-700' : '!bg-white border-slate-200/60 hover:border-slate-300 hover:shadow-md'}`}>
                          ...
                          <button onClick={() => handleToggle(item.key)} className={`w-14 h-8 rounded-full transition-colors relative flex items-center shadow-inner ${formData[item.key] ? `!bg-${item.color}-500` : (isDark ? '!bg-slate-800' : '!bg-slate-200')}`}>
                ...
                {activeTab === 'security' && (
                  <form onSubmit={handleSave} className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                    <div>
                      ...
                      <p className={`font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Update your password and secure your worker account.</p>
                    </div>
